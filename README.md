...
This service provides a simple REST API to generate unique numbers for named sequences like `ORDER`, `USER`, or `INVOICE`. Any part of this application can call it and get back the next number ? guaranteed to be unique even when multiple services or threads are calling at the same time.
